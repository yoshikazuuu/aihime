...
const getLyrics = async (songUrl) => {
  const response = await axios.get(LYRICS_ENDPOINT, {
    params: {
      url: songUrl,
    },
  });
...
export default async (_, res) => {
  try {
    ...
    const lyricsAPI = await getLyrics(data.songUrl);

    let lyric = lyricsAPI.lines.map((line) => line.words).join(" ");
    ...
    res.status(200).json({
      isPlaying: true,
      lyric: "",
      error: "Song doesn't have any lyrics.",
    });
  }
};
